...
> **v2.1.1 最终交付状态：** 教学级完整 Harness、Trace v3、最终离线验收矩阵、全新目录 tarball smoke 与真实 Provider 只读验收均已通过；脱敏后的文件与命令输出最多向下一轮保留 12,000 字符，重复读取会被纠正，CLI 会明确报告常见 Provider 故障。正式分发入口为 [GitHub v2.1.1 Release](https://github.com/fighting321321/ai4se-coding-agent-harness/releases/tag/v2.1.1)。
...
- 托管分发：[GitHub v2.1.1 Release](https://github.com/fighting321321/ai4se-coding-agent-harness/releases/tag/v2.1.1) 与 `ai4se-harness-2.1.1.tgz`。学校 GitLab 仅保留开发历史备份。
...
## 托管交付：GitHub v2.1.1 Release

课程检查入口：[GitHub v2.1.1 Release](https://github.com/fighting321321/ai4se-coding-agent-harness/releases/tag/v2.1.1)。依据助教补充说明，本项目采用“CLI + 托管平台 Release”方式交付；学校 GitLab 因网络稳定性仅保留开发历史备份。

从 Release 下载 `ai4se-harness-2.1.1.tgz` 后，推荐直接使用 Node.js 24 自带的 npm 全局安装：

```powershell
npm install --global .\ai4se-harness-2.1.1.tgz
ai4se-harness smoke
```

如果已经配置好 pnpm 11.14.0，也可以使用 `pnpm add --global .\ai4se-harness-2.1.1.tgz`。若 pnpm 报 `ERR_PNPM_NO_GLOBAL_BIN_DIR`，可改用上面的 npm 命令，或执行一次 `pnpm setup` 并重新打开终端。
...
`@ai4se/harness` 2.1.1 是可安装的 ESM 包，提供类型入口、共享任务运行器、会话运行器、`runOfflineSmoke` 和 `ai4se-harness` CLI。以下 PowerShell 命令会构建 tarball，在新目录离线安装，然后分别验证 ESM 导入和已安装 CLI：
