...
async function readTaskConfig(configPath: string): Promise<TaskConfigResult> {
...
export async function preflightHarnessTaskConfig(
  ...
  const configured = await readTaskConfig(configPath);
...
export async function runHarnessTask(options: RunHarnessTaskOptions): Promise<RunTaskResult> {
  const configured = await readTaskConfig(options.configPath);
  if (!configured.ok) {
    return configured;
  }
