...
- [x] 至少比较三个候选方向，优先从治理、反馈闭环、工具分发/运行控制、记忆与上下文中选择。
- [x] 每个方案都从用户价值、机制深度、确定性可测性、实现风险、演示效果和项目周期六方面评估。
- [x] 为每个方案列出至少三个可由代码实现的深度特性，不能只列 prompt、Skill 或配置内容。
- [x] 写明最终选择、未选方案的拒绝理由及可能保留为基础实现的部分。
- [x] 定义主要贡献的成功判据，以及无需真实 LLM、网络和真实 key 的验证方式。
- [x] 记录至少一轮方案比较引发的设计变化，使 `SPEC_PROCESS.md` 累计达到至少三轮关键迭代。
...
- [x] `AGENT_LOG.md` 的时间线连续，记录 Skill、prompt/context、人工干预和前五个提交 hash。
- [x] `SPEC_PROCESS.md` 至少包含三轮真实关键迭代，每轮都有问题、回答、AI 建议、本人判断和设计变化。
- [x] 目标用户、真实问题、使用场景、价值陈述和非目标已经由本人确认。
- [x] 已明确为何不用现成 Coding Agent，以及自研 Harness 的结构性价值。
- [x] Agent 能力边界、六维最低实现、主要贡献维度和确定性测试思路完整。
- [x] 至少三个候选方案已比较，采纳、拒绝和修改理由可追溯。
- [x] 仓库中尚无实现代码、工程骨架、依赖配置或测试代码。
- [x] 写入 T01 结论摘要和已确认设计输入；未创建 `SPEC.md`，且按负责人要求不处理 T02。
- [x] 明确下一提交只清空 `guiding.md`；本对话不执行推送、MR 或合并。
- [x] 明确任何后续任务均不继续使用当前 T01 分支；本对话在清空 `guiding.md` 后结束。
