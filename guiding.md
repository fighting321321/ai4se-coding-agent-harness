...
- [ ] 目标用户是谁，以及其当前工作方式。
- [ ] 用户面对的真实问题、发生频率和现有代价。
- [ ] 至少三个具体使用场景，每个场景写明触发、输入、期望结果和失败影响。
- [ ] 一段陌生人能在 30 秒内理解的价值陈述。
- [ ] 为什么用户不直接使用现成 Coding Agent；自研 Harness 必须解决哪一个结构性缺口。
- [ ] 明确非目标，排除“万能 Coding Agent”、无边界自主执行和与主要问题无关的功能。
- [ ] 记录至少一轮因用户质询而发生的设计修改，保留修改前后差异。
...
- [ ] Agent 可以执行的任务和明确禁止执行的任务。
- [ ] 动作与工具：最小工具集合、输入输出、工作目录边界和失败返回原则。
- [ ] 客观反馈：测试、lint、类型检查或构建等信号如何判定成功、失败与环境错误。
- [ ] 危险动作：哪些动作 `allow`、`ask`、`deny`，审批前后必须保持什么不变量。
- [ ] 记忆与上下文：跨会话保存什么、何时检索、何时清除、哪些敏感信息永不保存。
- [ ] 决策主循环：上下文组织、单次 LLM 调用、动作解析、执行、观察回灌和停机的最低闭环。
- [ ] 配置与可观测性：安全默认值、配置校验、日志脱敏和 Trace 最低要求。
- [ ] 明确六个维度都必须由可运行代码提供最低实现，并说明如何用 mock LLM 做确定性验证。
- [ ] 记录至少一轮因机制边界不清而发生的设计修订。
