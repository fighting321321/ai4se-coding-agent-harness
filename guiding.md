...
- [x] 写明 T04 目标、G3 边界、冷启动隔离规则和文件职责。
- [x] 将主控预审修复、输入冻结、冷启动试做、问题分类、文档修订、复验、G3 审计和清空规划拆成独立提交。
- [x] 明确第二个提交先修复已知 T03 问题，不把它们冒充冷启动发现。
- [x] 为每个提交写明修改范围、验证命令、证据和提交信息。
- [x] 明确 T04 独立分支和 T05 只能在 G3 通过后开始。
...
- [x] T13 的 G4 命令只运行 `Files` 中明确创建的测试路径；不得引用未声明的 `tests/integration/runtime/core-loop.test.ts`。
- [x] T15 的三项演示命令展开为三个真实路径，不得保留 `demos/mechanisms/<name>.test.ts`。
- [x] 将同时包含编写测试、实现、运行、评审、提交、清空 guiding 或合并 MR 的复合 Step 拆成单一动作。
- [x] 每个拆分后的 Step 明确 `(2–5 min)`、执行命令或可观察产物、预期结果。
- [x] 重点检查 T05、T06、T07、T09、T10、T11、T13、T14、T16、T17、T18、T19、T20 的复合步骤。
- [x] 两阶段评审、修复 Critical、全量回归、业务提交、更新台账、清空 guiding、等待 Pipeline 和合并 MR 必须分开。
- [x] 不改变 SPEC 产品范围、接口语义、Txx 分工、依赖 DAG 或主要贡献。
...
- [x] 主开发智能体类型和陌生智能体类型，证明二者不同。
- [x] 全新会话创建方式、无 memory/历史的检查方法和验证时间。
- [x] `SPEC.md`、`PLAN.md` 的 commit hash、blob hash、文件 hash 和行数。
- [x] 提供给陌生智能体的文件清单，明确没有额外文件或口头补充。
- [x] 原始 prompt：选择 1–2 个 Task 试做；不确定时停止提问；不得猜测。
- [x] 试做隔离位置和禁止合入正式代码的清理/保留规则。
- [x] 原始问题记录模板：时间、任务、文件/行、原文、暂停状态、是否继续。
- [x] 结果分类模板和预期差距比较模板。
...
- [x] 使用协议中声明的不同类型陌生智能体和全新会话；截图 1 证明空白新对话，完整模型名称与 memory/个性化状态未获证实并已如实记录。
- [x] 仅记录截图显示的 `SPEC`/`PLAN` 附件和原始回复；未记录项目历史、总结或口头解释为输入，原始回复中“SPEC.md（未直接上传）”与截图事实并列保留。
- [x] 记录其自行选择的 T05 及选择理由。
- [x] 原样记录全部疑问、暂停点、错误理解、尝试命令和观察结果。
- [x] 保存试做产物清单（两张截图、原始文本回复；Gemini 报告产物/diff 为“无”），未提交正式源码。
- [x] 记录试做停止原因、可直接记录的截图/接收时间、已完成步骤和未完成步骤；真实响应起止时间未直接记录。
- [x] 记录首次试做结束前未提示已知答案或引导绕过疑问。
...
- [x] 将每条发现分类为 SPEC 缺陷、PLAN 缺陷、两者不一致、环境缺口或智能体误读。
- [x] 为每项分类引用具体文件、行、原始问题和预期行为。
- [x] 比较试做产物与 PLAN 预期的文件、接口、测试、命令和停止条件。
- [x] 判断哪些问题会阻塞新鲜 subagent，哪些只是表达偏好。
- [x] 至少确认并处理一个真实隐含假设或规约缺陷；如果首次试做没有发现，G3 不得通过，必须更换 Task 或重新验证。
- [x] 对智能体误读给出文本证据，不得为了凑缺陷而修改正确规约。
...
- [x] 只修复已分类并获负责人统一批准的 F-01，不顺手扩展功能。
- [x] 每项修改记录修订前文本、修订后文本、原因、影响的 REQ/US/Txx 和批准结论。
- [x] 证据未支持 SPEC 缺陷，故 SPEC 版本、追踪矩阵和批准记录保持不变；PLAN 先以 1.0.2 同步冷启动边界，再依据失败复验补强为 1.0.3，T05 文件归属、接口返回值、步骤与依赖均不变。
- [x] 1.0.3 明确 T04 可在 G3/OPEN-03 未满足时把 T05 Step 1–5 标为“模拟/未执行”后静态推演，Gate 只阻止正式 worktree、写入、命令、提交和进度记录；不得伪造 G3 证据或版本裁决。
- [x] 1.0.3 明确测试中的 `.js` 是 TypeScript ESM 的有意导入说明符，兼容配置应解析到 `.ts` 源码并在构建后对应 `.js`；不新增文件、不提前选择工具链版本。
- [x] 修复后重新扫描路径一致性、类型命名、占位符、原子步骤和需求覆盖。
- [x] 被拒绝的智能体建议保留拒绝理由，不从记录中删除。
...
> 实际状态：已尝试三个 OpenCode 新会话。第一次选择了 T06/T07 后上下文压缩失败；第二次在摘要阶段因 `grep` 被拒绝而失败；第三次完成同一 T05 文本推演，但仍误读 G3 边界和 `.js`/`.ts`，故下列验收项不能勾选。项目负责人已停止后续外部模型复验，提交 7 未形成通过证据。
...
实际：未达到；不得提交为“验证有效性”或进入提交 8 的 G3 批准步骤。
...
- [x] 不同类型智能体、隔离输入及实际可核验证据均已记录；无法核验的 Gemini memory/完整模型名也如实标为证据缺口。
- [x] T05/T06/T07 的选择、试做、疑问、暂停点、错误理解和产物差距完整记录；通过的 Gate 只依赖已审计事实。
- [x] 真实规约缺陷已修订；同 Task 自动复验未成功，项目负责人审阅全部失败证据后明确接受剩余风险并批准 G3，未伪装为复验通过。
- [x] SPEC/PLAN 当前状态一致，无新增占位符；T05–T20 路径、接口和验证命令保持可执行设计。
- [x] 负责人已批准修订与风险接受，并在 SPEC/PLAN 写入 G3 状态；版本保持 SPEC 1.0.0、PLAN 1.0.3。
- [x] `AGENT_LOG.md` 记录提交 1–7 hash、验证环境、人工干预和经验。
- [x] 明确 G3 通过后仍须先合入 `dev`，再从最新 `dev` 创建 T05 独立分支；本分支不写实现。
- [x] 明确下一提交只清空 `guiding.md`，随后将本分支合入 `dev`，禁止 squash。
...
- [ ] 确认提交 1–8 已按顺序存在；失败复验已按负责人风险接受路径关闭，G3 审计无未完成项。
