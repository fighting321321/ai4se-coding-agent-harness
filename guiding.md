...
- [x] 写明 T02 的目标、范围、硬性约束和文件职责。
- [x] 固定 `SPEC.md` 的目标章节结构。
- [x] 将规约编写拆成一次提交一个可审阅成果的顺序步骤。
- [x] 为每个提交写明文件范围、内容要求、验证命令和提交信息。
- [x] 明确 T02 独立分支、合并前清空 `guiding.md`、T03 另建分支的生命周期。
...
- [x] 文档状态、版本、适用范围、权威来源和变更规则。
- [x] 问题陈述：小型开发团队的决策如何在成员、Agent、会话和分支之间断裂，以及为什么 Git/MR/CI 不能独自解决。
- [x] 目标用户、次要角色、当前工作方式、三个已批准使用场景和失败代价。
- [x] 30 秒价值陈述、产品目标、量化成功信号和明确非目标。
- [x] 术语表：Decision、DecisionVersion、Scope、ContextSnapshot、TaskRun、Rebaseline、Conflict、HITL、Observation、Trace。
- [x] 至少五个符合 INVEST 的用户故事；每条包含角色、目标、价值、前置条件和独立验收结果。
- [x] 用户故事至少覆盖：记录决策、按范围选择决策、生成可复现快照、检测过期快照、冲突审批、运行 Agent、查看 Trace。
...
- [x] 决策登记与不可变版本历史。
- [x] 决策范围匹配、优先级和状态筛选。
- [x] `ContextSnapshot` 的确定性生成、内容摘要和指纹。
- [x] 任务启动、快照绑定、Agent 运行状态和停机原因。
- [x] 写入前的快照过期检测、决策差异展示和 Rebaseline。
- [x] 互斥决策检测、结构化冲突和 HITL 审批。
- [x] 受限文件/命令工具、客观反馈传感器和失败回灌。
- [x] Trace、审计查询和 WebUI 展示。
- [x] 凭据状态、录入、更新和清除的用户流程。
...
- [x] 决策：自研 Agent 主循环如何组织上下文、调用单次 LLM 接口、解析动作、执行、回灌和停机。
- [x] 工具：注册、schema、参数校验、工作区围栏、超时、输出限制和结构化异常。
- [x] 记忆与上下文：决策版本存储、范围选择、快照生成、过期检测、Rebaseline 和清除策略。
- [x] 治理：`allow/ask/deny`、审批状态机、动作绑定、拒绝/超时和审计。
- [x] 反馈：版本、契约、测试/lint/类型检查/构建传感器，失败分类、回灌、重试和升级。
- [x] 配置与可观测性：安全默认值、schema 校验、脱敏日志、Trace 和预算。

**A 方向边界必须写清：**

- [x] LLM 抽象只负责单次模型调用，允许注入脚本化 mock。
- [x] 不使用 LangChain AgentExecutor、AutoGen、CrewAI、LlamaIndex Agent 或宿主 Agent Runner 充当内核。
- [x] prompt、Skill、配置和规则文件只是内容物，不计入 Harness 内核工作量。
- [x] 每个机制都说明不使用真实 LLM 时如何直接构造输入并确定性断言结果。
- [x] 三项强制演示：危险动作拦截、失败驱动修正、版本化决策上下文行为。

**主要贡献必须写深：**

- [x] 不可变 DecisionVersion、确定性 Scope 匹配、选择/排除理由、规范序列化和快照指纹。
- [x] 旧快照写前阻断、决策 diff、Rebaseline 后重新规划、旧动作失效。
- [x] 同范围互斥决策的结构化冲突和 HITL，且审批绑定原始冲突与动作参数。
- [x] 六条 T01 成功判据全部转成可观测、可测试要求。
...
- [x] 性能：冷启动、快照生成、查询、写前校验、Trace 写入和 WebUI 响应的目标及测量数据规模。
- [x] 可靠性：原子写入、崩溃恢复、幂等、超时、重试上限、预算耗尽和明确停机。
- [x] 安全：工作区围栏、路径逃逸、符号链接、命令 allowlist、敏感信息脱敏和最小权限。
- [x] 可用性：首次启动、错误提示、审批可理解性、无障碍和用户不得被假成功误导。
- [x] 可观测性：每轮决策、动作、观察、反馈、审批、快照版本和停机原因可追溯。
- [x] 兼容性与资源：目标操作系统、CPU 架构、运行时版本范围、并发、磁盘、日志保留和成本上限。

**凭据威胁模型必须包含：**

- [x] 资产、攻击者、入口、信任边界和滥用场景。
- [x] 禁止凭据进入源码、Git 历史、命令历史、日志、Trace、记忆、错误信息和前端回显。
- [x] 首选安全存储方案及目标平台适配；`.env` 只作为可选来源并说明明文风险。
- [x] 隐藏录入、仅显示配置状态、更新、清除、无效凭据和存储不可用流程。
- [x] 日志脱敏、异常清理、测试替身、凭据扫描和泄露后的轮换处置。
- [x] 每条威胁对应预防、检测和恢复措施以及验证方式。
...
- [x] 关联用户故事和功能模块。
- [x] 可观察的通过条件和失败条件。
- [x] 计划使用的验证层级：单元、集成、机制演示、端到端、CI、人工可用性或线上 smoke test。
- [x] 是否依赖真实 LLM、网络或 key；核心机制行必须全部为否。
- [x] 对应未来任务 T05–T20 的建议归属，但不得在本阶段编写 `PLAN.md`。

**风险与未决问题必须包含：**

- [x] 版本化决策范围匹配错误、冲突爆炸、快照存储增长和 Rebaseline 循环。
- [x] 跨平台命令与路径隔离、符号链接、凭据存储兼容性和 WebUI/Runtime 信任边界。
- [x] LLM 输出解析失败、供应商不可用、传感器环境错误、Trace 泄密和部署成本。
- [x] 规约复杂度超过个人项目周期、主要贡献被基础模块稀释和演示不可理解。
- [x] 每项风险的概率、影响、预防、检测、恢复和最迟处理任务。
- [x] 每个未决问题的候选选项、当前倾向、负责人、最迟决策阶段及未决时的保守默认行为。
...
- [x] 项目负责人逐节阅读 `SPEC.md`，对每节记录“批准”“修改后批准”或“拒绝并退回”。
- [x] `SPEC_PROCESS.md` 记录重要 AI 建议及项目负责人采纳、拒绝、修改的理由，不得只写最终结论。
- [x] 核对用户故事、功能模块、架构组件、数据实体、需求编号和验收项之间的一致性。
- [x] 核对 T01 六条主要贡献成功判据全部进入正式规约和验收矩阵。
- [x] 核对所有功能都有输入、行为、输出、边界、错误处理和验收条件。
- [x] 核对凭据、分发、WebUI、CI、部署、mock 测试和三项机制演示没有遗漏。
- [x] 删除重复、矛盾、无法验证或未经确认的要求；不得遗留空章节和占位符。
- [x] 在 `SPEC.md` 写入版本号、批准时间、批准状态和进入 T03 的前置条件。

**T02 审计记录：**

- [x] `AGENT_LOG.md` 记录提交 1–8 的 hash、逐节审阅、人工干预、验证命令和经验。
- [x] `SPEC_PROCESS.md` 写入 T02 最终批准摘要，以及相对 T01 的新增、细化和变更。
- [x] 明确 G1 只有在本提交完成且项目负责人批准全部章节后才通过。
- [x] 明确下一提交只清空 `guiding.md`，随后创建目标为 `dev` 的 T02 MR。
