...
> **v2.1.2 稳定性补丁（2026-08-09）：** 修复恢复型动作反馈、敏感动作误判、重复恢复停机保护与无 `dist` 冷启动测试解析，并保留 v2.1.1 的交付边界。

> **v2.1.1 托管迁移（2026-08-01）：** 保留 v2.1.0 功能与安全边界，将最终公开分发入口迁移到稳定的 GitHub Release，学校 GitLab 仅作为开发历史备份。

> **v2.1.0 可用性更新（2026-08-01）：** 脱敏后的文件和成功命令输出最多向下一轮保留 12,000 字符；连续重复读取同一未修改文件不会再次执行；CLI 会区分 Provider 限流、凭据、网络和服务端故障。
