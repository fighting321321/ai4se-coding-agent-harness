...
- T04 冷启动试做是上述禁令的隔离验证活动：允许陌生智能体在不连接项目工作区的外部网页或一次性沙箱中进行静态推演，并在回答正文中生成一次性文本草案或失败证据；这些内容不得写回项目工作区、不得作为可执行产物保留，也不构成 G3 豁免。环境没有终端、Git 或写入能力时，应如实报告并继续可验证的静态分析，不得伪造执行结果。
...
| 当前批准状态 | 项目负责人已批准 1.0.0、授权 1.0.1 执行一致性修订，并于 2026-07-18 统一批准 1.0.2 冷启动边界修订；G2 已通过 |
| 实现权限 | 未开放；T04 只可进行隔离试做，G3 通过前禁止正式实现 |
...
T04 的“试做”只允许以下活动：在不连接项目工作区的外部网页或一次性沙箱中静态推演 Task 的文件、接口、步骤、测试、命令和停止条件；在回答正文中生成不可执行、不得写回仓库的一次性文本草案；或者如实保存因工具、输入、Gate 或规约歧义产生的失败证据。若环境没有终端、Git、文件写入或命令执行能力，智能体应列出真实能力，在能力范围内继续静态分析，并把所有未执行动作明确标为“未执行”。
...
- Gate：G2 保持通过；G3 仍未通过，必须用修订后的 SPEC/PLAN 在另一个全新会话复验同一 T05，且原始阻塞消失后才能审计。
