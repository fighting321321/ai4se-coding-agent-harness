...
describe("最终交付审计", () => {
  it("将 v2.1.0 Release 和普通交互 CLI 作为托管交付入口", () => {
    ...
    const releaseNotes = readFileSync(
      join(repositoryRoot, "docs/releases/v2.1.0-release-notes.md"),
      "utf8"
    );

    expect(readme).toContain("GitLab v2.1.0 Release");
    expect(readme).toContain("ai4se-harness-2.1.0.tgz");
    expect(readme).toContain("ai4se-harness smoke");
    expect(readme).toContain("直接运行 `ai4se-harness`");
    expect(readme).toContain("/-/releases/v2.1.0");
    expect(readme).not.toContain("总控发布后填写");
    expect(readme).not.toContain("真实 Pages URL：**待最终审计核验**");
    expect(spec).toContain("GitLab Release");
    expect(plan).toContain("v2.1.0");
    expect(releaseNotes).toContain("AI4SE Coding Agent Harness v2.1.0");
