...
| --- | --- |
| 文档版本 | 1.0.0 |
| 当前状态 | 项目负责人已逐节批准；G1、G2、G3 已通过；需求版本仍为 1.0.0 |
...
| 实现权限 | 有条件开放：T04 必须先合入 `dev`；T05 只能从最新 `dev` 创建独立分支/worktree，且须在 OPEN-03 获批后才能安装依赖或创建工程骨架 |
...
G3 于 2026-07-18 01:33:39 +08:00 由项目负责人批准。批准采用风险接受例外：负责人审阅 T04 的首次试做、三次失败复验、PLAN 1.0.2/1.0.3 修订及剩余风险后，明确要求停止重复外部模型复检并完成 G3。该决定不把失败复验改写为成功；剩余风险是新的陌生智能体仍可能误读冷启动边界。该状态更新不改变任何需求、架构或验收条款，因此 `SPEC.md` 版本保持 1.0.0。
